...
function AlertasContent() {
  const [alerts, setAlerts] = useState<Alert[]>(userAlerts.map((a) => ({ ...a, visible: true })));
  ...
  const handleToggle = (id: string, checked: boolean) => {
    setAlerts((prev) => prev.map((a) => (a.id === id ? { ...a, active: checked } : a)));
  };
  ...
  return (
    <div className={styles.container}>
      ...
      <Typography as="p" className={styles.subtitle}>
        Configura alertas para recibir notificaciones cuando los precios cambien en tus zonas de
        interes.
      </Typography>
      ...
      {visibleAlerts.map((alert) => (
        <WiredCard key={alert.id} elevation={2} className={styles.alertCard}>
          <div className={styles.alertInfo}>
            ...
            <Typography as="p">
              Tipo: {alert.type === "alquiler" ? "Alquiler" : "Compraventa"} | Umbral:{" "}
              {alert.threshold}% | Frecuencia: {alert.frequency}
            </Typography>
          </div>
          <div className={styles.alertActions}>
            <div className={styles.toggleRow}>
              ...
              />
              <Typography as="span">{alert.active ? "Activa" : "Inactiva"}</Typography>
            </div>
      ...
      {!showForm ? (
        <WiredButton onClick={() => setShowForm(true)}>Nueva alerta</WiredButton>
      ) : (
        <WiredCard elevation={2} className={styles.formCard}>
          ...
          <div className={styles.formFields}>
            <div className={styles.formGroup}>
              ...
              <WiredSearch placeholder="Buscar zona..." onChange={() => {}} />
            </div>

            <div className={styles.formGroup}>
              <Typography as="span">Tipo</Typography>
              <WiredCombo selected={newType} onSelected={(v) => setNewType(v)}>
            ...
            <div className={styles.formGroup}>
              <Typography as="span">Umbral de variacion: {newThreshold}%</Typography>
              <WiredSlider
                value={newThreshold}
                min={1}
                max={20}
            ...
            <div className={styles.formGroup}>
              ...
              <WiredCombo selected={newFrequency} onSelected={(v) => setNewFrequency(v)}>
          ...
          <div className={styles.formActions}>
            ...
            <WiredButton onClick={() => setShowForm(false)}>Cancelar</WiredButton>
          </div>
        </WiredCard>
      )}
    </div>
