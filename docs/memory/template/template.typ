...
#let template(
  ...
  grid(
    ...
    rect(width: 100%, height: 1fr,fill: rgb("F0F0F0"), inset: 10pt)[
      ...
      #v(20pt)
      #if program != none {
        text(font: "Arial", size: 14pt, fill: marine, program)
        v(5pt)
      }
      #if area != none {
        text(font: "Arial", size: 14pt, fill: marine, area)
        v(10pt)
      }
      #if tutor != none {
        text(font: "Arial", size: 12pt, weight: "bold", fill: marine, [Tutor/a de TF])
        linebreak()
        text(font: "Arial", size: 14pt, fill: marine, tutor)
        v(5pt)
      }
      #if professor != none {
        text(font: "Arial", size: 12pt, weight: "bold", fill: marine, [Profesor/a responsable de la asignatura])
        linebreak()
        text(font: "Arial", size: 14pt, fill: marine, professor)
        v(10pt)
      }
      #v(1fr)
  ...
  set page(
    ...
    header: [
      #pad(left: -2.4cm, [
        #let logo-width = 63pt
        #let logo-height = logo-width * (70.55 / 101.54)
        #grid(
          columns: (logo-width, 40%, 1fr),
          rows: (logo-height),
          gutter: 5pt,
          rect(width: 100%, height: 100%, inset: 0pt, fill: aqua, [
            #image("assets/logo.svg", width: 100%)
          ]),
          rect(stroke: (top: 2pt + aqua, bottom: 2pt + aqua), width: 100%, height: 100%, inset: 5pt)[
            #align(horizon, [
