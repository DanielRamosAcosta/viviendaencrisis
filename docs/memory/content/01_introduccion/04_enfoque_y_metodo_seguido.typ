...
El primero es el _Diseño Centrado en el Usuario_ (DCU), que estructura la fase de definición del producto. Se llevarán a cabo actividades de investigación con usuarios ---entrevistas y análisis de necesidades--- que fundamentarán la arquitectura de la información, la definición de perfiles de uso y el diseño de prototipos de baja y alta fidelidad. Estas actividades siguen las prácticas del _Design Thinking_, que organiza el proceso creativo en torno a la comprensión empática del problema antes de proponer soluciones técnicas.
...
El cuarto eje es la validación con usuarios reales al cierre del desarrollo. Una vez alcanzado el alcance del producto mínimo viable (MVP), se realizará una evaluación con personas usuarias de prueba para verificar la usabilidad de la interfaz y la comprensibilidad de las visualizaciones, cuyos resultados se incorporarán como evidencia de validación en las conclusiones del trabajo.
...
Esta investigación preliminar, cuyo resultado se documenta en el @cap:materiales, deberá determinar qué formatos de distribución ofrece cada fuente (ficheros CSV, JSON, APIs REST, servicios WFS/WMS), qué granularidad territorial y temporal está disponible en cada caso, y qué volumen de datos es necesario procesar para el alcance funcional del MVP. Solo tras este análisis será posible tomar decisiones fundadas sobre la arquitectura del proceso de ingesta, la frecuencia de actualización de los datos en la plataforma y el modelo de almacenamiento más adecuado. Se contempla asimismo la posibilidad de incorporar, como dato de contraste complementario, indicadores de portales inmobiliarios privados obtenidos mediante extracción puntual de datos públicos, siempre dentro del marco legal aplicable y sin evadir medidas técnicas de protección.
