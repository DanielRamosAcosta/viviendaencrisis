---
id: VC-0048
title: Creación de User Personas
status: To Do
assignee: []
created_date: '2026-03-04 19:22'
updated_date: '2026-03-15 10:46'
labels:
  - ux/ui
milestone: m-1
dependencies: []
priority: high
ordinal: 3109.375
---

## Description

<!-- SECTION:DESCRIPTION:BEGIN -->
Crear perfiles de usuario (personas) basados en los datos del estudio de público objetivo:
- Persona 1: Inquilino joven que busca entender si le están cobrando de más
- Persona 2: Periodista/investigador que necesita datos abiertos para artículos o informes
- Persona 3: Activista o miembro de asociación vecinal que quiere denunciar abusos y ver tendencias

Realizar en FigJam con datos demográficos, motivaciones, frustraciones y objetivos.
<!-- SECTION:DESCRIPTION:END -->

## Definition of Done
<!-- DOD:BEGIN -->
- [ ] #1 All test suite is passing
<!-- DOD:END -->
